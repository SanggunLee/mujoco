...
Model compilation
   ...
   위에서 언급했듯이 사용자는 MJCF라는 XML 파일 형식으로 MuJoCo 모델을 정의합니다. 그런 다음 이 모델은 내장 컴파일러에 의해 저수준 데이터 구조 :ref:`mjModel` 로 컴파일되며 , 이는 교차 인덱싱되고 런타임 계산에 최적화됩니다. 컴파일된 모델은 바이너리 MJB 파일로 저장할 수도 있습니다.
